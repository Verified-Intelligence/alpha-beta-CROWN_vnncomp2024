...
if [[ "$CATEGORY" == "vggnet16_2022" || "$CATEGORY" == "vggnet16_2023" || "$CATEGORY" == "vggnet_16" ]]; then
...
echo
temp_file=$(mktemp)
if [[ "$CATEGORY" == "nn4sys" || "$CATEGORY" == "nn4sys_2023" ]]; then
	prepare_timeout=275
elif [[ "$CATEGORY" == "vggnet16_2022" || "$CATEGORY" == "vggnet16_2023" || "$CATEGORY" == "vggnet16" ]]; then
	prepare_timeout=90
elif [[ "$CATEGORY" == "tllverifybench" || "$CATEGORY" == "tllverifybench_2023" ]]; then
	prepare_timeout=90
elif [[ "$CATEGORY" == "collins_yolo_robustness" || "$CATEGORY" == "collins_yolo_robustness_2023" ]]; then
	prepare_timeout=180
else
	prepare_timeout=45
fi
