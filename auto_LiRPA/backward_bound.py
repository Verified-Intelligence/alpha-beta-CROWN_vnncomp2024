...
def batched_backward(self: 'BoundedModule', node, C, unstable_idx, batch_size,
    ...
    auto_batch_size = AutoBatchSize(self.bound_opts['crown_batch_size'], self.device, 0.8)
        ...
        while i < unstable_size:
            if isinstance(unstable_idx, tuple):
                unstable_idx_batch = tuple(
                    u[i : i + auto_batch_size.batch_size]
                    for u in unstable_idx
                )
                unstable_size_batch = len(unstable_idx_batch[0])
            else:
                unstable_idx_batch = unstable_idx[i : i + auto_batch_size.batch_size]
...
def concretize(self, batch_size, output_dim, lb=None, ub=None,
    ...
    for i in range(len(roots)):
        ...
        if average_A and isinstance(roots[i], BoundParams):
            ...
            ).expand(roots[i].lA.shape) if (roots[i].lA is not None) else None
            uA = roots[i].uA.mean(
                node_start.batch_dim + 1, keepdim=True
            ).expand(roots[i].uA.shape) if (roots[i].uA is not None) else None
        else:
            lA, uA = roots[i].lA, roots[i].uA
        if not isinstance(roots[i].lA, eyeC) and not isinstance(roots[i].lA, Patches):
            lA = roots[i].lA.reshape(output_dim, batch_size, -1).transpose(0, 1) if (lA is not None) else None
        if not isinstance(roots[i].uA, eyeC) and not isinstance(roots[i].uA, Patches):
            uA = roots[i].uA.reshape(output_dim, batch_size, -1).transpose(0, 1) if (uA is not None) else None
        if hasattr(roots[i], 'perturbation') and roots[i].perturbation is not None:
            if isinstance(roots[i], BoundParams):
                ...
                lb = lb + roots[i].perturbation.concretize(
                    ...
                    sign=-1, aux=roots[i].aux) if (lA is not None) else None
                ub = ub + roots[i].perturbation.concretize(
                    roots[i].center.unsqueeze(0), uA,
                    sign=+1, aux=roots[i].aux) if (uA is not None) else None
            else:
                lb = lb + roots[i].perturbation.concretize(
            ...
            def _add_constant(A, b):
                if isinstance(A, eyeC):
                    add_b = fv.view(batch_size_, -1)
                elif isinstance(A, Patches):
                    add_b = A.matmul(fv, input_shape=roots[0].center.shape)
                elif type(roots[i]) == BoundInput:
                    add_b = A.matmul(fv.view(batch_size_, -1, 1)).squeeze(-1)
                else:
                    add_b = A.matmul(fv.view(-1, 1)).squeeze(-1)
                return add_b if b is None else (b + add_b)

            lb = _add_constant(lA, lb) if lA is not None else None
            ub = _add_constant(uA, ub) if uA is not None else None
